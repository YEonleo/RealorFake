...
backbone = config['MODEL'][model_name]['backbone']
wandb_run_name = backbone
wandb_run_name+="_" + train_serial
...
if __name__ == '__main__':
    ...
    X_train, X_val, Y_train, Y_val = SplitDataset(img_dir = DATA_DIR,
                 ...
                 aug_datasets=config['DATASET']['aug_datasets'],
                 )
        
    train_dataset = CustomDataset(X = X_train, y = Y_train, backbone=backbone , aug = config['DATASET']['augmentation'])
    val_dataset = CustomDataset(X = X_val, y = Y_val, backbone=backbone, aug = 0)#test 셋이므로 augmentation X
